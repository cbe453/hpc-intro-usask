...
best way to transfer them to Plato?

1. `scp -r data abc123@plato.usask.ca:~/`
2. `rsync -ra data abc123@plato.usask.ca:~/`
3. `rsync -raz data abc123@plato.usask.ca:~/`
4. `tar -cvf data.tar data;`

   `rsync -raz data.tar abc123@plato.usask.ca:~/`
5. `tar -cvzf data.tar.gz data;`

   `rsync -ra data.tar.gz abc123@plato.usask.ca:~/`
